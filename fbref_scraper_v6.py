...
def get_player_ids(current_player, table_name, stats_dict):
    if current_player in stats_dict["standard_stats"]:
        total_matches = 0
        for key in stats_dict["standard_stats"][current_player]:
            if key in stats_dict[table_name][current_player]:
                if stats_dict["standard_stats"][current_player][key] == stats_dict[table_name][current_player][key]:
                        total_matches += 1
                        if total_matches >= 5:
                            player_id = stats_dict["standard_stats"][current_player]["player_id"]
                            return player_id
                        ...
                        if stat_tag == "matches" or stat_tag == "ranker":
                            ...
                            if stat_tag == "player":
                                ...
                                if table_name != "standard_stats":
                                    ...
                                    stats_dict[table_name][current_player].update({"player_id":matched_player_id})
                                    
    player_info = get_player_table(stats_dict)
    stats_dict.update(player_info) 
    all_stats = reformat_stats(stats_dict, league_dict, season_dict, team_dict)
